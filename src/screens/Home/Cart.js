...
import { Appbar, Button, Colors, Divider, IconButton, List, Paragraph, Searchbar, Surface } from 'react-native-paper';
...
const Cart = ({ navigation, items, createOrder, loading }) => {
	...
	const renderContent = () => (
		...
		>
			<List.Item
				title="Item"
				right={() => <Text style={{ marginTop: 15, fontSize: 14 }}>{currencyFormat(getsum())}</Text>}
			/>
			<Divider />
			<List.Item
			...
			<List.Item
				...
				right={() => (
					<Text style={{ marginTop: 5, fontSize: 18, fontWeight: '600' }}>
						{getsum() > 50000 ? currencyFormat(0 + getsum()) : currencyFormat(0 + getsum())}
					</Text>
				)}
			/>
			<Divider />
			...
			/> */}
			<Paragraph style={{textAlign: 'center', color: Colors.red500}}>Hi cnnnnnnnnnnnnnnnnnnn Pharmacy, you delivery location is currently out of DrugStoc free delivery zone. Shipment may attract delivery fee. Our Customer service will contact you when your orders has been confirmed</Paragraph>
	...
	const renderHeader = () => (
		<View>
			<List.Item title="Shipping Address" onPress={() => sheetRef.current.snapTo(2)} />
			<Divider />
		</View>
	);
	...
	const renderItem = (data) => <CartList key={data.index} id={data.index} data={data.item} />;
	// const renderFooter = () => (
	...
	const renderHeaderComp = () => (
		...
		<List.Subheader style={{ backgroundColor: '#F4F4F4' }}>
			{items.data.length} Items in your cart {getsum() > 0 ? "Total Cost:" + currencyFormat(getsum()) : null}
		</List.Subheader>
		</List.Section>
		</View>
	);
	...
	return (
		<Surface style={{ flex: 1 }}>
			<SafeAreaView>
				...
				/>
				<Surface style={{ paddingHorizontal: 20, position: 'absolute', bottom: 0, width: "100%", elevation: 0 }}>
			<Button
				mode="contained"
				loading={loading}
				disabled={loading}
				// disabled={getsum() == 0}
				style={{ padding: 5, borderRadius: 50, marginTop: 10, marginBottom: 40 }}
