...
import { sign_in, sign_out, skip_for_now, verify_account, resend_otp } from '../store/action/auth.action';
import { startloading, stoploading, ui_start_loading, ui_stop_loading } from '../store/action/loading.action';
...
export const login_user_to_app = (payload) => {
    return async dispatch => {
        ...
        await axois.post(`${urlendPoint}/api/login/`, payload).then(resp => {
            ...
            Alert.alert("Some thing went wrong", "Incorrect username or password. Please check to be sure you have the correct details.")
        })
    }
}
...
export const register_a_new_user_to_app = (payload) => {
    ...
    return async dispatch => {
        dispatch(ui_start_loading())
        await axois.post(`${urlendPoint}/api/register`, payload).then(resp => {
            dispatch(ui_stop_loading())
...
export const resend_otp_again = (payload) => {
    ...
    return async dispatch => {
        dispatch(ui_start_loading())
        await axois.post(`${urlendPoint}/api/resend-otp/`, payload).then(resp => {
            dispatch(ui_stop_loading())
            dispatch(resend_otp(payload))
        }).catch(err => {
            console.log(err.response)
            dispatch(ui_stop_loading())
            Alert.alert("Something went wrong", err.response.data.message || 'Failed to send Otp')
        })
    }
}
...
export const verify_a_new_user_has_a_valid_otp = (payload) => {
    return async dispatch => {
        await axois.post(`${urlendPoint}/api/verify/`, payload).then(resp => {
...
export const getting_users_profile = () => {
    return async dispatch => {
        let logged_in_user = await AsyncStorage.getItem('token');
        await axois.get(`${urlendPoint}/api/user/`, {
            "headers": {
...
export const getting_users_profile_account = () => {
    return async dispatch => {
        let logged_in_user = await AsyncStorage.getItem('token');
        await axois.get(`${urlendPoint}/api/products/user_profile`, {
            "headers": {
                "Content-Type": 'application/json',
                "Authorization": `Token ${logged_in_user}`
            }
...
export const upload_user_licences = (payload) => {
    return async dispatch => {
        ...
        dispatch(ui_start_loading())
        let logged_in_user = await AsyncStorage.getItem('token');
        await axois.post(`${urlendPoint}/api/documents/`, payload, {
            "headers": {
                "Content-Type": 'application/json',
                "Authorization": `Token ${logged_in_user}`
            }
